...
class tpGd{
    ...
    public function open($imgname){
        ...
        if('gif' == $this->info['type']){
            require(dirname(__FILE__).'/tpGIF.php');
            $this->gif = new tpGIF($imgname);
     ...
     */
    public function thumb($width, $height, $type = tpImage::IMAGE_THUMB_SCALE){
        ...
        switch ($type) {
            /* 等比例缩放 */
            case tpImage::IMAGE_THUMB_SCALING:
                //原图尺寸小于缩略图尺寸则不进行缩略
            ...
            /* 居中裁剪 */
            case tpImage::IMAGE_THUMB_CENTER:
                //计算缩放比例
            ...
            /* 左上角裁剪 */
            case tpImage::IMAGE_THUMB_NORTHWEST:
                //计算缩放比例
                $scale = max($width/$w, $height/$h);
            ...
            /* 右下角裁剪 */
            case tpImage::IMAGE_THUMB_SOUTHEAST:
                //计算缩放比例
                $scale = max($width/$w, $height/$h);
            ...
            /* 填充 */
            case tpImage::IMAGE_THUMB_FILLED:
                //计算缩放比例
                if($w < $width && $h < $height){
                    $scale = 1;
                } else {
            ...
            /* 固定 */
            case tpImage::IMAGE_THUMB_FIXED:
                $x = $y = 0;
                break;
     ...
     */
    public function water($source, $locate = tpImage::IMAGE_WATER_SOUTHEAST){
        //资源检测
        ...
        switch ($locate) {
            /* 右下角水印 */
            case tpImage::IMAGE_WATER_SOUTHEAST:
                $x = $this->info['width'] - $info[0];
                $y = $this->info['height'] - $info[1];
                break;

            /* 左下角水印 */
            case tpImage::IMAGE_WATER_SOUTHWEST:
                $x = 0;
                $y = $this->info['height'] - $info[1];
                break;

            /* 左上角水印 */
            case tpImage::IMAGE_WATER_NORTHWEST:
                $x = $y = 0;
                break;

            /* 右上角水印 */
            case tpImage::IMAGE_WATER_NORTHEAST:
                $x = $this->info['width'] - $info[0];
                $y = 0;
                break;

            /* 居中水印 */
            case tpImage::IMAGE_WATER_CENTER:
                $x = ($this->info['width'] - $info[0])/2;
                $y = ($this->info['height'] - $info[1])/2;
                break;

            /* 下居中水印 */
            case tpImage::IMAGE_WATER_SOUTH:
                $x = ($this->info['width'] - $info[0])/2;
                $y = $this->info['height'] - $info[1];
                break;

            /* 右居中水印 */
            case tpImage::IMAGE_WATER_EAST:
                $x = $this->info['width'] - $info[0];
                $y = ($this->info['height'] - $info[1])/2;
                break;

            /* 上居中水印 */
            case tpImage::IMAGE_WATER_NORTH:
                $x = ($this->info['width'] - $info[0])/2;
                $y = 0;
                break;

            /* 左居中水印 */
            case tpImage::IMAGE_WATER_WEST:
                $x = 0;
                $y = ($this->info['height'] - $info[1])/2;
                break;
    ...
    public function text($text, $font, $size, $color = '#00000000', 
        $locate = tpImage::IMAGE_WATER_SOUTHEAST, $offset = 0, $angle = 0){
        //资源检测
